...
//START: Integration of an LED Strip or Ring
...
//END: Integration of an LED Strip or Ring
...
// START: Integration of LED Strip or Ring
...
// END: Integration of LED Strip or Ring
...
{
  lsrLoopCountWait = 5; // Geschwindigkeit der Animation, je größer desto langsamer
...
if (lsrAnimationMode == 2 && loopCountdown == 0)
{
  ...
  do
  {
    for (i = 0; i < strip.numPixels(); i++)
    {
      lsrHueCalc = 19000 / (strip.numPixels() - 1) / (strip.numPixels() - 1);
    ...
    if (tempCard.nfcFolderSettings.folder == 0) {
      if (activeModifier != NULL) {
        if (activeModifier->getActive() == tempCard.nfcFolderSettings.mode) {
          // START: Don't leak memory when removing modifiers  //////////////////////////
          delete activeModifier;
          // END: Don't leak memory when removing modifiers  ////////////////////////////
          activeModifier = NULL;
